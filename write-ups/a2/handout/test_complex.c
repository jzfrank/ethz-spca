...
static int double_equals_epsilon(double a, double b)
{
  if (a > b)
  {
    // a > b case
...
{
  struct complex a = {1, 2}, b = {3, 4}, c = {0, 0};
  struct complex res;
  ...
  res = complex_add(a, b);
  assert(double_equals_epsilon(res.real, 4.0) == 1);
  assert(double_equals_epsilon(res.imaginary, 6.0) == 1);

  res = complex_subtract(a, b);
  assert(double_equals_epsilon(res.real, -2.0) == 1);
  assert(double_equals_epsilon(res.imaginary, -2.0) == 1);

  res = complex_multiply(a, b);
  assert(double_equals_epsilon(res.real, -5) == 1);
  assert(double_equals_epsilon(res.imaginary, 10.0) == 1);

  res = complex_divide(a, b);
  assert(double_equals_epsilon(res.real, 0.44) == 1);
  assert(double_equals_epsilon(res.imaginary, 0.08) == 1);

  // Test the "divide by zero" case.  On modern Intel
  // machines and Linux, this causes the returned
  // struct complex to contain the value "NAN" in both the
